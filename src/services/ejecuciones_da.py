# src/services/ejecuciones_da.py
# Validación y persistencia específicas de Análisis Dimensional puro
# (MCDM, sin metaheurística).
#
# A diferencia de las familias PSO/BA/ACO, aquí no hay matriz editable ni
# iteraciones: la matriz es fija (ver MATRIZ_FIJA en src/algoritmos/da.py)
# y el único parámetro de entrada es w.
#
# Archivo propio, no comparte código con ejecuciones_topsis.py ni
# ejecuciones_moorav.py — mismo criterio de responsabilidad separada ya
# aplicado en PSO/BA/ACO.

from src.models.models import db, Ejecucion
from src.algoritmos.da import MATRIZ_FIJA

N_CRITERIOS = len(MATRIZ_FIJA[0])


def validar_entrada_da(payload: dict) -> dict:
    """
    Valida y normaliza el payload JSON del frontend para DA puro.
...
def guardar_ejecucion_da(user_id: int, params: dict, datos: dict) -> Ejecucion:
    """
    Persiste el resultado de una ejecución de DA puro. No reutiliza
    ...
    datos   : dict devuelto por ejecutar_da()
    """
    ejecucion = Ejecucion(
        algoritmo='DA',
        fk_user=user_id,

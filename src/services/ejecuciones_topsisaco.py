# src/services/ejecuciones_topsisaco.py
# Validación, plantilla Excel y exportación específicas de TOPSIS-ACO.
#
# TOPSIS-ACO recibe w más los parámetros de ACO (alpha, beta, rho, Q,
# n_ants, T). La persistencia (guardar_ejecucion) es genérica y se
# reutiliza desde ejecuciones_pso.py.
...
def validar_entrada_topsisaco(payload: dict) -> dict:
    """
    Valida y normaliza el payload JSON del frontend para TOPSIS-ACO.
...
def generar_plantilla_excel_topsisaco(n_criterios: int = MIN_CRITERIOS,
                                      n_alternativas: int = MIN_ALTERNATIVAS) -> BytesIO:
    """
    Genera un libro de 4 hojas para capturar un experimento de TOPSIS-ACO offline:
    ...
    h.set_column('A:A', 90)
    h.write('A1', 'Plantilla de experimento TOPSIS-ACO', fmt_titulo)
    instrucciones = [
        ...
        '2. Hoja "Vectores": capture el peso w, un valor por criterio. TOPSIS-ACO usa '
        'w para ponderar la matriz normalizada antes de calcular las distancias a la '
        'solución ideal y anti-ideal, en el ranking que se calcula una sola vez al '
        ...
        '5. Guarde el archivo y súbalo en la sección Laboratorio de TOPSIS-ACO.',
    ]
    for i, t in enumerate(instrucciones):
        h.write(i + 2, 0, t, fmt_texto)
    h.write(len(instrucciones) + 4, 0, '__ALGORITMO__:TOPSISACO', fmt_oculto)
...
def parsear_plantilla_excel_topsisaco(archivo) -> dict:
    """
    Lee una plantilla de TOPSIS-ACO llenada y devuelve el payload listo
    para validar_entrada_topsisaco.
    """
    try:

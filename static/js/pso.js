...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderMatriz() {
    ...
    // Filas de vectores por criterio (visualmente diferenciadas)
    matrizVectores.innerHTML = '';
    const etiquetas = { w: 'w (peso)', r1: 'R1', r2: 'R2' };
    for (const prefijo of ['w', 'r1', 'r2']) {
      const tr = document.createElement('tr');
      const th = document.createElement('th');
      th.className = 'border border-blue-200 px-2 py-1 bg-blue-50 text-left text-blue-900';
      th.textContent = etiquetas[prefijo];
      tr.appendChild(th);
      for (let c = 0; c < nCriterios; c++) {
        const td = document.createElement('td');
  ...
  document.getElementById('psoForm').addEventListener('submit', (e) => e.preventDefault());

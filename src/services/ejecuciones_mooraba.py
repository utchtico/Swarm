# src/services/ejecuciones_mooraba.py
# Validación, plantilla Excel y exportación específicas de MOORA-BA.
#
# Igual que DA-BA, MOORA-BA SÍ recibe pesos por criterio (w): el método
# MOORA los usa para ponderar la matriz normalizada antes de calcular la
# puntuación. No recibe r1/r2 (exclusivo de la familia PSO). La persistencia
...
def validar_entrada_mooraba(payload: dict) -> dict:
    """
    Valida y normaliza el payload JSON del frontend para MOORA-BA.
...
def generar_plantilla_excel_mooraba(n_criterios: int = MIN_CRITERIOS,
                                    n_alternativas: int = MIN_ALTERNATIVAS) -> BytesIO:
    """
    Genera un libro de 4 hojas para capturar un experimento de MOORA-BA offline:
    ...
    h.set_column('A:A', 90)
    h.write('A1', 'Plantilla de experimento MOORA-BA', fmt_titulo)
    instrucciones = [
        '1. Hoja "Matriz": capture la matriz de decisión. Filas = alternativas (A), '
        'columnas = criterios (C). Solo edite las celdas en azul.',
        '2. Hoja "Vectores": capture el peso w, un valor por criterio. MOORA-BA usa w '
        'para ponderar la matriz normalizada antes de calcular la puntuación MOORA.',
        ...
        '5. Guarde el archivo y súbalo en la sección Laboratorio de MOORA-BA.',
    ]
    for i, t in enumerate(instrucciones):
        h.write(i + 2, 0, t, fmt_texto)
    h.write(len(instrucciones) + 4, 0, '__ALGORITMO__:MOORABA', fmt_oculto)
...
def parsear_plantilla_excel_mooraba(archivo) -> dict:
    """
    Lee una plantilla de MOORA-BA llenada y devuelve el payload listo para

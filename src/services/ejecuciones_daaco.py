# src/services/ejecuciones_daaco.py
# Validación, plantilla Excel y exportación específicas de DA-ACO.
#
# DA-ACO recibe w (pondera el índice de similitud del Análisis Dimensional,
# calculado una sola vez) más los parámetros de ACO (alpha, beta, rho, Q,
# n_ants). La persistencia (guardar_ejecucion) es genérica y se reutiliza
# desde ejecuciones_pso.py.
...
def validar_entrada_daaco(payload: dict) -> dict:
    """
    Valida y normaliza el payload JSON del frontend para DA-ACO.
    ...
    return {'matriz': matriz, 'w': w, 'alpha': alpha, 'beta': beta,
            'rho': rho, 'Q': Q, 'n_ants': n_ants, 'T': T}


def generar_plantilla_excel_daaco(n_criterios: int = MIN_CRITERIOS,
                                  n_alternativas: int = MIN_ALTERNATIVAS) -> BytesIO:
    """
    Genera un libro de 4 hojas para capturar un experimento de DA-ACO offline:
      Instrucciones | Matriz | Vectores (solo w) | Parametros
    """
    ...
    h.set_column('A:A', 90)
    h.write('A1', 'Plantilla de experimento DA-ACO', fmt_titulo)
    instrucciones = [
        ...
        '2. Hoja "Vectores": capture el peso w, un valor por criterio. DA-ACO usa w '
        'para ponderar el índice de similitud del Análisis Dimensional, calculado '
        'una sola vez al inicio del experimento.',
        ...
        '(número de hormigas) y T, que aquí representa tanto el número de repeticiones '
        ...
        '5. Guarde el archivo y súbalo en la sección Laboratorio de DA-ACO.',
    ]
    for i, t in enumerate(instrucciones):
        h.write(i + 2, 0, t, fmt_texto)
    h.write(len(instrucciones) + 4, 0, '__ALGORITMO__:DAACO', fmt_oculto)
...
def parsear_plantilla_excel_daaco(archivo) -> dict:
    """
    Lee una plantilla de DA-ACO llenada y devuelve el payload listo para

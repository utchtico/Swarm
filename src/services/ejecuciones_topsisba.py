# src/services/ejecuciones_topsisba.py
# Validación, plantilla Excel y exportación específicas de TOPSIS-BA.
#
# Igual que DA-BA y MOORA-BA, TOPSIS-BA SÍ recibe pesos por criterio (w):
# el método TOPSIS los usa para ponderar la matriz normalizada antes de
# calcular las distancias a la solución ideal/anti-ideal. No recibe r1/r2
# (exclusivo de la familia PSO). La persistencia (guardar_ejecucion) es
# genérica y se reutiliza desde ejecuciones_pso.py.
...
def validar_entrada_topsisba(payload: dict) -> dict:
    """
    Valida y normaliza el payload JSON del frontend para TOPSIS-BA.

    Estructura esperada:
    {
      "matriz": [[...], ...],   # a filas x n columnas
      "w": [...],               # longitud n — pondera la matriz normalizada
...
def generar_plantilla_excel_topsisba(n_criterios: int = MIN_CRITERIOS,
                                     n_alternativas: int = MIN_ALTERNATIVAS) -> BytesIO:
    """
    Genera un libro de 4 hojas para capturar un experimento de TOPSIS-BA offline:
    ...
    h.set_column('A:A', 90)
    h.write('A1', 'Plantilla de experimento TOPSIS-BA', fmt_titulo)
    instrucciones = [
        '1. Hoja "Matriz": capture la matriz de decisión. Filas = alternativas (A), '
        'columnas = criterios (C). Solo edite las celdas en azul.',
        '2. Hoja "Vectores": capture el peso w, un valor por criterio. TOPSIS-BA usa w '
        'para ponderar la matriz normalizada antes de calcular las distancias a la '
        ...
        '5. Guarde el archivo y súbalo en la sección Laboratorio de TOPSIS-BA.',
    ]
    for i, t in enumerate(instrucciones):
        h.write(i + 2, 0, t, fmt_texto)
    h.write(len(instrucciones) + 4, 0, '__ALGORITMO__:TOPSISBA', fmt_oculto)
...
def parsear_plantilla_excel_topsisba(archivo) -> dict:
    """
    Lee una plantilla de TOPSIS-BA llenada y devuelve el payload listo
    para validar_entrada_topsisba.
    """
    try:

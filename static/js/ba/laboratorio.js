// static/js/ba/laboratorio.js
// Laboratorio BA: matriz de decisión dinámica (sin pesos w ni vectores R1/R2,
// BA no los usa), parámetros alpha/gamma, stepper, tabs y gráfica de
// convergencia. Independiente de static/js/pso/laboratorio.js a propósito:
// BA tiene un esquema de parámetros completamente distinto al de PSO.

document.addEventListener('DOMContentLoaded', function () {
  const CFG = window.ALGO_CONFIG || {};
  const ALGORITMO = CFG.algoritmo || 'BA';
  const ENDPOINT  = CFG.endpoint  || '/api/algoritmos/ba';
  const TIENE_W   = CFG.tiene_w === true;  // default: false (BA puro no usa w; DA-BA sí)
  ...
  // ===================== MATRIZ (sin w, sin R1/R2 — BA no los usa) =====================
  ...
  function renderMatriz() {
    ...
    // Fila de pesos w — solo para variantes que sí la usan (DA-BA, no BA puro)
  ...
  function construirPayload() {
    ...
    // w solo existe en el DOM y aplica para variantes que lo usan (DA-BA)
    if (TIENE_W) {
      const w = [];
      for (let c = 0; c < nCriterios; c++) {
